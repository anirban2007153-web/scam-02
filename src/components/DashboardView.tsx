...
interface DashboardViewProps {
  ...
  onClear: () => void;
  onResetText: () => void;
  onSimulateOnKeypad: () => void;
  onSwitchView: (view: ActiveView) => void;
  scanRecords: ScanRecord[];
  totalAnalyzed: number;
  highRiskCount: number;
  safetyTipsStudied: number;
}
...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  return (
    <div className="space-y-8" id="view-web-dashboard">
      ...
      >
        <div className="relative z-10 max-w-3xl">
          ...
          <p className="mt-2 text-slate-300 text-sm md:text-base leading-relaxed">
            Understand suspicious messages before you trust them. Inspect incoming SMS copy for phishing vectors, deceptive domains, credential harvesting, and urgent panic language. Try the exact same indicator engine in rich{' '}
            <span className="text-cyan-300 font-semibold">Smartphone Web Mode</span> or accessible{' '}
            <span className="text-cyan-300 font-semibold">Keypad Feature Phone Mode</span>.
          </p>
          <div className="mt-5 flex flex-wrap gap-3">
            <button
              type="button"
              onClick={scrollToAnalyzer}
              className="px-4 py-2.5 rounded-lg bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold text-sm transition shadow-lg shadow-cyan-500/20 flex items-center space-x-2 focus:outline-none cursor-pointer"
            >
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-lg bg-slate-800/90 hover:bg-slate-700 text-slate-200 border border-slate-700 font-medium text-sm transition flex items-center space-x-2 focus:outline-none cursor-pointer"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4" id="dashboard-stats-row">
        ...
        <div className="bg-[#121d36]/90 border border-slate-800 rounded-xl p-4 sm:p-5 relative shadow-md">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-rose-950/60 text-rose-300 border border-rose-900">
              DEMO
            </span>
          </div>
          <div className="mt-2 flex items-baseline space-x-2">
        ...
        <div className="bg-[#121d36]/90 border border-slate-800 rounded-xl p-4 sm:p-5 relative shadow-md">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-emerald-950/60 text-emerald-300 border border-emerald-900">
              DEMO
            </span>
          </div>
          <div className="mt-2 flex items-baseline space-x-2">
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5">
            <button
              ...
              className={`group text-left px-3.5 py-2.5 rounded-xl border transition flex items-center justify-between focus:outline-none cursor-pointer ${
                currentScenario.id === 'kyc'
                  ? 'bg-rose-950/40 border-rose-500 shadow-sm ring-1 ring-rose-500'
                  : 'bg-slate-900/90 hover:bg-slate-800/90 border-rose-900/50 hover:border-rose-500/70'
              }`}
            >
              <div>
                ...
                <span className="text-[10px] text-slate-400 font-mono">{SCENARIOS.kyc.subtitle}</span>
              </div>
              <span className="px-1.5 py-0.5 rounded text-[9px] font-mono font-bold bg-rose-950 text-rose-400 border border-rose-800">
                HIGH
              </span>
            </button>
            ...
            >
              <div>
                <span className="text-xs font-semibold text-rose-300 group-hover:text-rose-200 block">
                  {SCENARIOS.electricity.name}
                </span>
                <span className="text-[10px] text-slate-400 font-mono">{SCENARIOS.electricity.subtitle}</span>
              </div>
              <span className="px-1.5 py-0.5 rounded text-[9px] font-mono font-bold bg-rose-950 text-rose-400 border border-rose-800">
                HIGH
              </span>
            ...
            <button
              ...
              className={`group text-left px-3.5 py-2.5 rounded-xl border transition flex items-center justify-between focus:outline-none cursor-pointer ${
                currentScenario.id === 'prize'
                  ? 'bg-amber-950/40 border-amber-500 shadow-sm ring-1 ring-amber-500'
                  : 'bg-slate-900/90 hover:bg-slate-800/90 border-amber-900/50 hover:border-amber-500/70'
              }`}
            >
              <div>
                <span className="text-xs font-semibold text-amber-300 group-hover:text-amber-200 block">
                  {SCENARIOS.prize.name}
                </span>
                <span className="text-[10px] text-slate-400 font-mono">{SCENARIOS.prize.subtitle}</span>
              </div>
              <span className="px-1.5 py-0.5 rounded text-[9px] font-mono font-bold bg-amber-950 text-amber-400 border border-amber-800">
                MEDIUM
              </span>
            </button>
            ...
            <button
              ...
              className={`group text-left px-3.5 py-2.5 rounded-xl border transition flex items-center justify-between focus:outline-none cursor-pointer ${
                currentScenario.id === 'doctor'
                  ...
                  : 'bg-slate-900/90 hover:bg-slate-800/90 border-emerald-900/50 hover:border-emerald-500/70'
              }`}
            >
              <div>
                <span className="text-xs font-semibold text-emerald-300 group-hover:text-emerald-200 block">
                  {SCENARIOS.doctor.name}
                </span>
                <span className="text-[10px] text-slate-400 font-mono">{SCENARIOS.doctor.subtitle}</span>
              </div>
        ...
        {/* Large Styled Textarea & Actions */}
        <div className="space-y-3">
          <div className="relative">
            <label className="sr-only" htmlFor="dashboard-sms-input">
              Suspicious SMS Text
            </label>
            <textarea
              ...
              rows={4}
              value={smsInput}
              onChange={e => onSmsInputChange(e.target.value)}
            ...
            {smsInput.length > 0 && (
              <button
                ...
                type="button"
                onClick={onClear}
                className="absolute right-3 top-3 text-xs font-mono text-slate-400 hover:text-slate-100 bg-slate-800/90 hover:bg-slate-700 px-2 py-1 rounded border border-slate-700 transition cursor-pointer"
              >
                Clear
              </button>
            )}
          ...
          <div className="flex flex-wrap items-center justify-between text-xs text-slate-400 px-1 gap-2">
            <div className="flex items-center space-x-3">
              <span className="font-mono text-slate-300" id="char-counter-dash">
                {smsInput.length} characters
              </span>
              <span className="text-slate-600">|</span>
              <span className="text-slate-400">Evaluation: Heuristic indicators check</span>
            </div>
            <span className="text-[11px] text-slate-500 font-mono">
              Zero telecommunication intercept • Client-side demo
            </span>
          </div>

          {/* Primary Action Bar with Glow Button & Keypad Bridge */}
          <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
            <div className="flex items-center space-x-2">
              <button
                type="button"
                onClick={onResetText}
                className="px-4 py-2 rounded-lg bg-slate-900 hover:bg-slate-800 text-slate-300 text-xs font-medium border border-slate-700 transition focus:outline-none cursor-pointer"
              >
                Reset Text
              </button>
              <button
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button
                type="button"
                onClick={onSimulateOnKeypad}
                title="Simulate this message on a basic keypad phone display"
                className="px-4 py-2.5 rounded-lg bg-indigo-950 hover:bg-indigo-900/80 text-cyan-300 text-xs font-semibold border border-indigo-700/70 transition flex items-center space-x-2 shadow-sm focus:outline-none cursor-pointer"
              >
                <Smartphone className="w-4 h-4 text-cyan-400" />
              ...
              <button
                ...
                type="button"
                onClick={onAnalyze}
                className="px-6 py-2.5 rounded-lg bg-gradient-to-r from-cyan-500 via-cyan-400 to-blue-500 hover:from-cyan-400 hover:to-blue-400 text-slate-950 font-bold text-sm transition shadow-lg shadow-cyan-500/30 flex items-center space-x-2 focus:outline-none active:scale-98 cursor-pointer"
              >
        ...
        <div ref={resultRef} className="mt-8 border-t border-slate-800 pt-7" id="dashboard-result-card">
          <div
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-slate-800/80">
              <div className="flex items-center space-x-3.5">
                ...
                <div>
                  ...
                  <p className="text-base sm:text-lg font-bold text-white mt-1 flex items-center space-x-2">
                    <span>Category:</span>
                    <span className="text-cyan-300" id="result-category-text">
              ...
              <div className="flex flex-col sm:items-end">
                <span
                  className="text-[11px] font-mono text-slate-400 bg-slate-950/80 px-2.5 py-1 rounded border border-slate-800"
                  id="result-verdict-tag"
                >
                  {isSafe
                    ? 'Heuristic Verdict: Verified Standard Indicators'
                    : 'Heuristic Verdict: Flagged by Suspicious Indicators'}
                </span>
                <span className="text-[10px] text-slate-500 font-mono mt-1">
                  Confidence: Heuristic Match
                </span>
              </div>
            </div>
            ...
            <div className="mt-6 p-4 rounded-xl bg-slate-900/90 border border-slate-700/80 text-xs text-slate-400 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              <div className="flex items-start space-x-2.5">
                <span className="text-cyan-400 font-mono text-sm mt-0.5">ℹ</span>
                <p className="text-[11px] sm:text-xs leading-relaxed text-slate-300">
                  <strong className="text-white">Prototype Notice:</strong> Demo analysis based on
                  suspicious message indicators for hackathon presentation purposes. Simulated frontend only.
                  Does not guarantee 100% detection, does not intercept live telecommunication traffic, and
                  does not replace official banking verification.
                </p>
              </div>
              <button
                type="button"
                onClick={onSimulateOnKeypad}
                className="flex-shrink-0 px-3.5 py-1.5 rounded-lg bg-cyan-950 hover:bg-cyan-900 border border-cyan-700/80 text-cyan-300 text-xs font-semibold flex items-center space-x-1.5 transition whitespace-nowrap focus:outline-none cursor-pointer"
              >
                <span>Simulate Warning on Keypad Phone →</span>
              </button>
            </div>
      ...
      {/* 4. DEMO HISTORY SECTION */}
      <div
        ref={historyRef}
        id="dashboard-history-section"
        className="bg-[#121d36] rounded-2xl border border-slate-800 p-6 shadow-xl space-y-4"
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 border-b border-slate-800">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Log of evaluated simulated SMS tests during this demonstration session.
            </p>
          </div>
          <div className="flex items-center space-x-2">
              ...
              {scanRecords.map(record => (
                <tr key={record.id} className="hover:bg-slate-800/30 transition">
                  ...
                  <td className="py-3 px-3.5 font-mono text-slate-400">
                    {record.indicatorsFlaggedCount} Indicators Flagged
                  </td>
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (record.scenarioKey) {
                          onSelectScenario(record.scenarioKey);
                        } else {
                          onSmsInputChange(record.fullText);
                          onAnalyze();
                        }
                        scrollToAnalyzer();
                      }}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          ...
          <p className="text-xs text-slate-400 mt-1">
            Review mock threat records and timestamps from this session.
          </p>
        </button>
        ...
        >
          ...
          <p className="text-xs text-slate-400 mt-1">
            Checklist and behavioral defense against social engineering.
          </p>
        </button>
      ...
      <div className="bg-gradient-to-r from-slate-900 via-[#121d36] to-slate-900 rounded-2xl border border-cyan-800/50 p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-xl">
        ...
        <button
          type="button"
          onClick={() => onSwitchView('keypad')}
          className="px-4 py-2.5 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-white font-semibold text-xs transition flex-shrink-0 flex items-center space-x-2 shadow-lg shadow-cyan-600/20 focus:outline-none cursor-pointer"
        >
      ...
      <div
        ref={awarenessRef}
        id="dashboard-awareness-section"
        className="bg-[#121d36] rounded-2xl border border-slate-800 p-6 md:p-8 space-y-6 shadow-xl"
      >
