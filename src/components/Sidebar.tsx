...
import { Shield, LayoutDashboard, MessageSquareText, Smartphone, History, BookOpen } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ activeView, onSelectView }) => {
  return (
    <aside className="w-full md:w-64 bg-[#0b1329]/95 border-b md:border-b-0 md:border-r border-slate-800 flex-shrink-0 flex flex-col justify-between sticky top-0 z-50 backdrop-blur-md">
      <div>
        {/* Brand Header */}
          ...
          >
            ...
            <div>
              <div className="flex items-center space-x-2">
                <span className="text-xl font-bold tracking-tight font-mono text-white">ScamShield</span>
              </div>
        ...
        <nav className="p-3 space-y-1.5 overflow-x-auto flex md:flex-col">
          ...
          <button
            ...
            type="button"
            onClick={() => onSelectView('dashboard')}
            className={`w-full flex items-center justify-between px-3.5 py-2.5 rounded-lg text-sm font-medium transition-all focus:outline-none cursor-pointer ${
          ...
          >
            <div className="flex items-center space-x-3">
              <MessageSquareText className={`w-5 h-5 ${activeView === 'analyzer' ? 'text-cyan-400' : 'text-slate-400'}`} />
          ...
          <button
            ...
            className={`w-full flex items-center justify-between px-3.5 py-2.5 rounded-lg text-sm font-medium transition-all border focus:outline-none cursor-pointer ${
              activeView === 'keypad'
                ? 'text-white bg-cyan-500/20 border-cyan-500/40 shadow-sm'
          ...
          >
            ...
            <span className="text-[10px] bg-cyan-500/20 text-cyan-300 px-1.5 py-0.5 rounded border border-cyan-500/40 font-mono">
              SIMULATION
            </span>
          </button>
          ...
          <button
            ...
            className={`w-full flex items-center space-x-3 px-3.5 py-2.5 rounded-lg text-sm font-medium transition-all focus:outline-none cursor-pointer ${
              activeView === 'history'
                ? 'text-white bg-cyan-500/10 border border-cyan-500/30 shadow-sm'
                : 'text-slate-400 hover:text-slate-100 hover:bg-slate-800/60'
            }`}
          ...
          >
            <BookOpen className={`w-5 h-5 ${activeView === 'awareness' ? 'text-cyan-400' : 'text-slate-400'}`} />
